...
void concatString(char** string, const char* text) {
    ...
    char* new_string = (char*)realloc(*string, new_size * sizeof(char));
    ...
    */
    while (( lsdir = readdir(dir) ) != NULL )
    {
        ...
        if ( !(!backfolder || !samefolder) ){
            ...
            char *extractor_dir = createString(strcat("./base/extractor_", folder_name));
            char *command = createString(strcat("mkdir ", extractor_dir));
